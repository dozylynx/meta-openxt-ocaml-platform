...
do_compile_prepend_class-opam32-native() {
    CLASSOVERRIDE="class-opam32-native"
}
...
def get_opam_uri_data(d, tag):
    ...
    for opam_uri in filter(lambda x:x.startswith('opam://'), src_uris):
        ...
        if ti == -1:
            ti = len(opam_uri)
        results.append(opam_uri[si+len(delimiter):ti])
    return results

def get_opam_switch_packages(d):
    """
    get the list of opam switch packages from SRC_URI data
    """
    tag = 'switch'
    src_uris = d.getVar("SRC_URI").split()
    switches = []
    for opam_uri in filter(lambda x:x.startswith('opam://'), src_uris):
        ...
        for tag in ['switch', 'repos']:
            ...
            if ti == -1:
                ti = len(opam_uri)
            result[tag] = opam_uri[si+len(delimiter):ti]
        if 'switch' in result and 'repos' in result:
            switches.append(result['switch'] + '-' + result['repos'])
    return switches
...
def get_opam_switch_depends(d):
    ...
    """
    switches = get_opam_switch_packages(d)
    suffix = '-cross-' + d.getVar("TARGET_ARCH")
    if bb.data.inherits_class('native', d):
        suffix = '-native'
    if bb.data.inherits_class('cross', d):
        suffix = '-native'
    if bb.data.inherits_class('opam32-native', d):
        suffix = '-opam32-native'
